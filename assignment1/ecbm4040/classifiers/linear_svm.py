...
def svm_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        # the prediction result in training
        scores = X[i].dot(W)
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    W_y = X.T
    output = np.dot(X, W)
    output -= np.reshape(output[np.arange(num_train), y], (num_train, 1))
    
    output = np.maximum(output + np.ones((num_train, num_classes)), 0) 
    
    output[np.arange(num_train), y] = 0    
    
    loss = np.sum(output)/num_train
    loss += reg * np.sum(W * W)
    ...
    output[output!=0] = 1
    output[np.arange(num_train), y] = -np.sum(output, axis=1)
