...
import time
from ecbm4040.image_generator import *
...
def LeNet(input_x, input_y,
    ...
    # norm layer 0
    norm_layer_0 = norm_layer(conv_layer_0.output())
    ...
    # norm layer 1
    norm_layer_1 = norm_layer(conv_layer_1.output())
    ...
    norm_layer2 = norm_layer(fc_layer_0.output())

    fc_layer_1 = fc_layer(input_x=norm_layer2.output(),
                          in_size=fc_units[0],
    ...
    # loss = cross_entropy(fc_layer_1.output(), input_y)
...
def train_step(loss, learning_rate=1e-3):
    with tf.name_scope('train_step'):
        ...
        #step = tf.train.RMSPropOptimizer(learning_rate=learning_rate, decay = 0.9, momentum = 0.1).minimize(loss)
...
def my_training_task4(X_train, y_train, X_val, y_val, 
    ...
    train_set = ImageGenerator(X_train, y_train)
    tsb = train_set.next_batch_gen(batch_size)
    ...
    with tf.Session() as sess:
        ...
        for epc in range(epoch):
            ...
            for itr in range(iters):
                ...
                training_batch_x, training_batch_y = tsb.__next__()
